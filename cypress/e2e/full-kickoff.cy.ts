...
describe('Full kickoff', () => {
  it('manual login then all page flows run automatically', () => {
    ...
    // })
    // cy.get('#ui-modal-backdrop', { timeout: 30000 }).should('not.exist')
    // cy.findTableRow(withoutKeyEmail).find('code').first().invoke('text').then((raw) => {
    //   const userId = String(raw || '').trim()
    //   expect(userId, 'seed userId').to.not.equal('')
    //   return userId
    // }).as('seedUserId')

    // cy.findTableRow(withKeyEmail).within(() => cy.get('button[title="Reset Password"]').click())
    ...
    // })
    // cy.findTableRow(withoutKeyEmail).should('be.visible')
    // cy.contains('th', 'Name').click()
    // cy.wait('@usersQuery').its('request.url').should('include', 'sortBy=name')
    // cy.contains('th', 'Name').click()
    // cy.wait('@usersQuery').its('request.url').should('include', 'sortOrder=desc')
    ...
    //   cy.get('#ui-modal-backdrop', { timeout: 30000 }).should('not.exist')
    //   cy.findTableRow(uid).within(() => cy.get('button[title="Edit Key"]').click())
    ...
    //   cy.get('#ui-modal-backdrop', { timeout: 30000 }).should('not.exist')
    //   cy.findTableRow(uid).contains('Disabled').should('be.visible')
    //   cy.findTableRow(uid).within(() => cy.get('button[title="Edit Key"]').click())
    //   cy.clickModalButton('Edit API Key', 'Rotate Key')
    //   cy.getModal('API Key').within(() => cy.get('button[title="Copy to clipboard"]').click())
    //   cy.get('#keys-generated-done').click()
    //   cy.get('#ui-modal-backdrop', { timeout: 30000 }).should('not.exist')
    //   cy.findTableRow(uid).within(() => cy.get('button[title="Delete key"]').click())
    //   cy.get('#keys-delete-confirm').click()
    //   cy.get('#ui-modal-backdrop', { timeout: 30000 }).should('not.exist')
    //   cy.openPageViaSidebar('/users', 'Users')
    //   cy.findTableRow(withoutKeyEmail).within(() => cy.get('button[title="Delete User"]').click())
    ...
    // cy.get('#ui-modal-backdrop', { timeout: 30000 }).should('not.exist')
    // cy.findTableRow(roleId).should('be.visible')
    // cy.findTableRow(roleId).within(() => cy.get('button[title="Delete Role"]').should('be.enabled').click())
    ...
    cy.findTableRow(dashPolicyAdminEmail, { searchInput: manageUsersSearch, searchTerm: dashPolicyAdminEmail, apiPattern: manageUsersApi }).within(() => cy.get('button[title="Disable User"]').click())
    cy.findTableRow(dashPolicyAdminEmail, { searchInput: manageUsersSearch, searchTerm: dashPolicyAdminEmail, apiPattern: manageUsersApi }).contains('Disabled').should('be.visible')
    cy.findTableRow(dashPolicyAdminEmail, { searchInput: manageUsersSearch, searchTerm: dashPolicyAdminEmail, apiPattern: manageUsersApi }).within(() => cy.get('button[title="Enable User"]').click())
    cy.findTableRow(dashPolicyAdminEmail, { searchInput: manageUsersSearch, searchTerm: dashPolicyAdminEmail, apiPattern: manageUsersApi }).contains('Active').should('be.visible')
    ...
    cy.get('@userAdminAccessUserId').then((uid) => {
      ...
      cy.contains('td,code', userId, { timeout: 20000 }).closest('tr').within(() => {
        cy.get('button[title="Delete key"]').click()
      })
      cy.get('#keys-delete-confirm').click()
      cy.get('#ui-modal-backdrop', { timeout: 20000 }).should('not.exist')
    ...
    cy.openPageViaSidebar('/settings/manage-users', 'Manage Users')
    for (const email of [dashViewerEmail, dashUserAdminEmail, dashPolicyAdminEmail]) {
    ...
    cy.openPageViaSidebar('/users', 'Users')
    cy.findTableRow(withKeyEmail, { searchInput: 'input[placeholder*="Search by name, email, or user ID"]', searchTerm: withKeyEmail }).find('button[title="Delete User"]').click({ force: true })
    cy.get('#users-delete-confirm').click()
    cy.get('#ui-modal-backdrop', { timeout: 30000 }).should('not.exist')
