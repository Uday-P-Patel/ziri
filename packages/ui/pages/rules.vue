...
<template>
  <div class="space-y-4">
    ...
    <template v-else>
      ...
      <div class="flex gap-2">
        <UiButton id="rules-templates-trigger" variant="outline" @click="handleOpenTemplateModal">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
          </svg>
          Templates
        </UiButton>
        <UiButton id="rules-ai-trigger" v-if="canGeneratePolicyWithAI" variant="outline" @click="showAIChatModal = true">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
          </svg>
          Create with AI
        </UiButton>
        <UiButton id="rules-create-trigger" v-if="canCreatePolicy" @click="showCreateModal = true">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="flex items-center justify-end gap-2" v-if="rules.length === 0 && !loading && !searchQuery && !filterEffect">
      <UiButton id="rules-templates-trigger-empty" variant="outline" @click="showTemplateModal = true">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
        </svg>
        Templates
      </UiButton>
      <UiButton id="rules-ai-trigger-empty" v-if="canGeneratePolicyWithAI" variant="outline" @click="showAIChatModal = true">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
        </svg>
        Create with AI
      </UiButton>
      <UiButton id="rules-create-trigger-empty" v-if="canCreatePolicy" @click="showCreateModal = true">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
        </svg>
        Create Policy
    ...
    >
      <template #empty-action>
        <UiButton id="rules-create-trigger-table-empty" v-if="canCreatePolicy && !searchQuery" @click="showCreateModal = true">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
          </svg>
          Create Policy
        </UiButton>
        <UiButton id="rules-ai-trigger-table-empty" v-if="canGeneratePolicyWithAI && !searchQuery" @click="showAIChatModal = true">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
          </svg>
          Create with AI
            ...
            >
              <div class="space-y-3">
                ...
                <div class="flex justify-end pt-2 border-t border-[rgb(var(--border))]">
                  <UiButton 
                    :id="`rules-template-use-${template.id}`"
    ...
    <UiModal v-model="showDeleteModal" title="Delete Policy" size="sm">
      <div class="space-y-4">
        ...
        <div class="flex gap-3 justify-end">
          ...
          <UiButton id="rules-delete-confirm" variant="danger" :loading="loading" @click="handleDeleteRule">
            Delete
          </UiButton>
        </div>
      </div>
